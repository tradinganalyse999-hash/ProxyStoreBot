...
from bot import bot
import traceback # ERROR DEKHAR JONNO
...
def register_handlers():
    ...
    def callback(call):
        try: # SOB TRY ER BHITOR DIYE DILAM
            if not call.message:
                bot.answer_callback_query(call.id)
                return
            msg_id = call.message_id # EITA IMPORTANT, AGE vul chilo
            ...
            # QUANTITY SYSTEM - SAFE SPLIT
            elif call.data.startswith("qty_"):
                parts = call.data.split("_", 3)
                _, category, safe_name, price = parts
                name = safe_name.replace("_", " ")
                qty = 1; price = float(price)
                total = price * qty
                text = f"🛒 *{name}*\n\n💎 প্রাইস: {price} BDT\nস্টক: 99+\n\nপরিমাণ: {qty}\nমোট: 💎 {total} BDT"
                bot.edit_message_text(text, chat_id=chat_id, message_id=msg_id, reply_markup=quantity_menu(category, safe_name, price, qty), parse_mode="Markdown")

            elif call.data.startswith("inc_"):
                parts = call.data.split("_")
                _, category, safe_name, price, qty = parts
                name = safe_name.replace("_", " ")
            ...
            print("CALLBACK ERROR:", e) # RAILWAY LOG E ERROR DEKHABE
            traceback.print_exc()
            bot.answer_callback_query(call.id, "⚠️ কিছু একটা সমস্যা হয়েছে")
    ...
    def process_all(message):
        ...
        if state["step"] == "custom_qty":
            ...
            total = price * qty
            text = f"🛒 *{name}*\n\n💎 প্রাইস: {price} BDT\nস্টক: 99+\n\nপরিমাণ: {qty}\nমোট: 💎 {total} BDT"
            bot.send_message(message.chat.id, text, reply_markup=quantity_menu(category, name.replace(" ", "_"), price, qty), parse_mode="Markdown")
