...
def register_handlers():
    ...
    def callback(call):
        msg_id = call.message.message_id # EITA THIK KORSI
        ...
        if call.data == "shop": bot.edit_message_text("🛒 ক্যাটাগরি সিলেক্ট করুন", chat_id=chat_id, message_id=msg_id, reply_markup=shop_menu())
            ...
            parts = call.data.split("_"); category = parts[1]; price = float(parts[-1]); name = " ".join(parts[2:-1]).replace("_", " ")
            balance = get_balance(user_id)
            if balance >= price:
                update_balance(user_id, -price); add_order(user_id, f"{category.upper()}: {name}", price)
            ...
            else: bot.answer_callback_query(call.id, "Already Done")
            ...
            orders = c.fetchall(); text = "📦 কোন অর্ডার নেই" if not orders else "📦 শেষ ২০ টি অর্ডার\n"+"\n".join([f"ID: {x[0]} | User: {x[1]}\nProduct: {x[2]}\nPrice: {x[3]} BDT | {x[4]}\n" for x in orders])
            ...
            for o in orders:
                markup = InlineKeyboardMarkup(); markup.add(InlineKeyboardButton("✅ Approve", callback_data=f"approve_{o[0]}"))
                bot.send_message(chat_id, f"🛒 Order ID: {o[0]}\nUser: {o[1]}\nProduct: {o[2]}\nPrice: {o[3]} BDT", reply_markup=markup)
        elif call.data.startswith("approve_"):
            if user_id!= ADMIN_ID: return
            order_id = int(call.data.split("_")[1]); bot.send_message(chat_id, f"📦 Order {order_id} এর জন্য প্রোডাক্ট কোড দিন"); user_state[user_id] = {"step": "admin_code", "order_id": order_id}
        elif call.data == "support": bot.edit_message_text("🆘 সাপোর্ট সেন্টার\n💬 সাপোর্ট: @PolasChandra\nWhatsApp: 01873565112\n⏰ ২৪/৭ এভেইলেবল", chat_id=chat_id, message_id=msg_id, reply_markup=main_menu())
    ...
    def process_all(message):
        ...
        if state["step"] == "admin_user_id":
            state["target_id"] = int(message.text); state["step"] = "admin_amount"
            bot.send_message(message.chat.id, "💰 কত BDT অ্যাড করবেন?")
        elif state["step"] == "admin_amount":
            amount = float(message.text); target_id = state["target_id"]
            ...
            bot.send_message(target_id, f"🎉 এডমিন আপনার একাউন্টে {amount} BDT অ্যাড করেছে"); del user_state[user_id]
            ...
            bot.send_message(message.chat.id, f"✅ Order {order_id} Approved and code sent to user"); del user_state[user_id]
        elif state["step"] == "amount":
            try: state["amount"] = float(message.text)
            except: bot.send_message(message.chat.id, "❌ ভুল Amount. শুধু নাম্বার দিন"); return
            ...
            if amount >= MIN_DEPOSIT_FOR_BONUS:
                ...
                if referrer: bot.send_message(referrer, f"🎉 অভিনন্দন!\nআপনার রেফার করা ইউজার {amount} টাকা ডিপোজিট করেছে!\nআপনি {REFERRAL_BONUS} BDT বোনাস পেয়েছেন\nনতুন ব্যালেন্স: {get_balance(referrer)} BDT")
            bot.send_message(message.chat.id,"✅ ডিপোজিট রিকোয়েস্ট পাঠানো হয়েছে। এডমিন চেক করে এপ্রুভ/রিজেক্ট করবে।"); del user_state[user_id]
