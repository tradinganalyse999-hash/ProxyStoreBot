...
def init_db():
    if USE_POSTGRES:
        c.execute('''CREATE TABLE IF NOT EXISTS users (user_id BIGINT PRIMARY KEY, balance FLOAT DEFAULT 0, referred_by BIGINT DEFAULT NULL, referral_count INT DEFAULT 0, total_referral_earning FLOAT DEFAULT 0)''')
        c.execute('''CREATE TABLE IF NOT EXISTS orders (id SERIAL PRIMARY KEY, user_id BIGINT, product TEXT, price FLOAT, status TEXT)''')
        c.execute('''CREATE TABLE IF NOT EXISTS stock (id SERIAL PRIMARY KEY, category TEXT, product_name TEXT, code TEXT, status TEXT DEFAULT 'available')''')
        c.execute('''CREATE TABLE IF NOT EXISTS deposits (id SERIAL PRIMARY KEY, user_id BIGINT, amount FLOAT, trx_id TEXT, status TEXT DEFAULT 'pending')''')
        c.execute('''CREATE TABLE IF NOT EXISTS referrals (id SERIAL PRIMARY KEY, referrer_id BIGINT, referred_id BIGINT UNIQUE, status TEXT DEFAULT 'pending')''')
    else:
        c.execute('''CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, balance REAL DEFAULT 0, referred_by INTEGER DEFAULT NULL, referral_count INTEGER DEFAULT 0, total_referral_earning REAL DEFAULT 0)''')
        c.execute('''CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, product TEXT, price REAL, status TEXT)''')
        c.execute('''CREATE TABLE IF NOT EXISTS stock (id INTEGER PRIMARY KEY AUTOINCREMENT, category TEXT, product_name TEXT, code TEXT, status TEXT DEFAULT 'available')''')
        c.execute('''CREATE TABLE IF NOT EXISTS deposits (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, amount REAL, trx_id TEXT, status TEXT DEFAULT 'pending')''')
        c.execute('''CREATE TABLE IF NOT EXISTS referrals (id INTEGER PRIMARY KEY AUTOINCREMENT, referrer_id INTEGER, referred_id INTEGER UNIQUE, status TEXT DEFAULT 'pending')''')
    conn.commit()
init_db()
...
def activate_referral_bonus(referred_id, deposit_amount=0):
    # 1. Minimum 10 TK check
    if deposit_amount < 10:
        return None
    # 2. Akbar bonus pay kina check (pending thakle e pabe)
    if USE_POSTGRES:
